...
2. ML correction layer (learns systematic bias the linear model misses).
   ...
   baseline, using [value_0h, value_24h, early_rate] as features. Its
   correction is reported explicitly, and feature_importances_ are
...
class DriftModel:
    ...
    def __init__(self, n_estimators: int = 200, random_state: int = 42):
        ...
        self.is_fit = False
        self.feature_names = ["value_0h", "value_24h", "early_rate"]

    def _features(self, df: pd.DataFrame) -> np.ndarray:
        rate = (df["value_24h"] - df["value_0h"]) / (T1 - T0)
        return np.column_stack([df["value_0h"], df["value_24h"], rate])

    def fit(self, historical: List[ParameterReading]):
    ...
    def predict_batch(
        ...
        for r in readings:
            ...
            if self.is_fit:
                x = self._features(pd.DataFrame([r.model_dump()]))
            ...
            explanation = (
                ...
                + (f", ML correction = {correction:+.3f} (trained on historical lots)"
                   if self.is_fit else ", no ML correction applied (insufficient training history, using pure physics baseline)")

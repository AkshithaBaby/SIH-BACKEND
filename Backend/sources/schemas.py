...
class ParameterReading(BaseModel):
    ...
    value_168h: Optional[float] = Field(
        None, description="Ground truth if known (e.g. during training/backtest). Omit at inference time."
    )
...
class BatchRequest(BaseModel):
    ...
    z_score_threshold: float = Field(3.5, description="Modified z-score threshold (Iglewicz & Hoaglin default).")
    safety_margin: float = Field(
        0.85, description="Fraction of safety_slope allowed before flagging (e.g. 0.85 = flag at 85% of the limit, for conservatism against false negatives)."
    )
